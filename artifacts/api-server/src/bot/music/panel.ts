...
async function buildPanelView(
  ...
  const isLive = Boolean(session?.current || (session && session.queue.length));
    ...
    try {
      ...
      if (saved && (saved.current || saved.queue.length)) {
        savedSession = {
          ...
          playbackSec: saved.playbackSec ?? 0,
          voiceChannelId: saved.voiceChannelId,
        };
        // queueLen = remaining queue (+ current is the one resuming, not in queue count for display)
        savedSession.queueLen = saved.queue.length;
      }
    } catch {
      /* optional */
    }
...
export async function refreshMusicPanel(
  ...
  try {
    ...
    const { embed, components } = await buildPanelView(client, guildId, cfg);
...
export async function publishMusicPanel(
  ...
  const { embed, components } = await buildPanelView(client, guildId, draft);
  const files = musicEmbedFiles();

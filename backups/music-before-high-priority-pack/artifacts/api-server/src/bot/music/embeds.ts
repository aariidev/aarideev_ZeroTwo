 ...
 */
/**
 * Controles del panel persistente del servidor (incluye "Añadir" + salir).
 * SNAPSHOT: antes del pack de alta prioridad.
 */
...
export function musicPanelEmbed(
  ...
  const emb = new EmbedBuilder()
    ...
    .setDescription(
      opts.current
        ? [
            ...
            progressBar(null, opts.current.durationSec),
            "",
    ...
    .setFooter({
      text: "Panel fijo · se actualiza al cambiar de pista",
      iconURL: client.user?.displayAvatarURL() ?? undefined,
    })
    .setTimestamp();

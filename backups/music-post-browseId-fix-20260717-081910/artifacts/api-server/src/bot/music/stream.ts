...
/**
 * Search YouTube with yt-dlp (avoids play-dl browseId / innertube breakage).
 */
export async function ytdlpSearchMeta(
  ...
  return new Promise((resolve) => {
    // Single line of fields separated by tabs (id, title, duration, thumbnail)
    const args = [
      `ytsearch1:${query}`,
      "--print",
    ...
    proc.on("close", (code) => {
      ...
      const [id, title, durationRaw, thumb] = line.split("\t");
      if (!id || !/^[\w-]{6,}$/.test(id)) {
        resolve(null);
        return;
      }
